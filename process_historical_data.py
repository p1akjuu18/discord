            ...
            try:
                ...
                if 'choices' in result and len(result['choices']) > 0:
                    ...
                    try:
                        ...
                        # 如果无法解析JSON，返回原始分析结果
                        return self._extract_basic_trading_info(analysis)
                    
            except Exception as e:
                print(f"第{attempt + 1}次尝试分析失败: {str(e)}")
    ...
    def process_json_files(self, data_dir: str, output_dir: str):
        ...
        for i, file_path in enumerate(json_files, 1):
            ...
            try:
                ...
                print(f"文件 {file_path.name} 分析完成")
...
def main():
    # 使用示例
    api_key = "sk-tuemouafroxshgugbsiunwxtuxsijnghmqgjogddxqylburk"  # 硅基流动API密钥
    processor = HistoricalDataProcessor(api_key)
    
    # 设置输入输出目录
    data_dir = "data/trading"  # 原始JSON文件目录
    output_dir = "data/analysis_results"  # 分析结果输出目录
